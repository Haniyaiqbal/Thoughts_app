...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            existing_email = User.objects.filter(email=current_user.email).exists()


            if existing_email:
                form.add_error('email', ValidationError('This email address is already in use.'))


                return render(request, 'journal/register.html', {'RegistrationForm': form})

            
            existing_username = User.objects.filter(username=current_user.username).exists()
            ...
            current_user.save()
            send_mail("Welcome to Thotsandprayers!", "Congratulations on creating your account", settings.DEFAULT_FROM_EMAIL, [current_user.email])
